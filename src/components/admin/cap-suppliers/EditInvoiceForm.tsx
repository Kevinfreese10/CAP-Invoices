...
import { s38ChartOfAccounts, capChartOfAccounts, s39ChartOfAccounts, goChartOfAccounts } from '@/lib/cap-chart-of-accounts';
import { ExtractedInvoice, Commission } from '@/lib/types';
import { format, parseISO, addDays, eachDayOfInterval, endOfMonth, isFriday, getMonth, isLastDayOfMonth, addMonths, endOfYear, startOfYear, getYear } from 'date-fns';
...
const formSchema = z.object({
  ...
  expenseType: z.enum(['CAP', 'S38', 'S39', 'GO']).optional(),
  paymentBatch: z.string().optional(),
  note: z.string().optional(),
  isPrivate: z.boolean().optional(),
});

function getUpcomingFridays(currentBatch?: string): { value: string; label: string }[] {
...
export default function EditInvoiceForm({ invoice, onSave, onCancel, onSaveAndApprove }: EditInvoiceFormProps) {
    const upcomingFridays = useMemo(() => getUpcomingFridays(invoice?.paymentBatch), [invoice?.paymentBatch]);
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                ...
                                <FormControl>
                                    ...
                                    <FormItem className="flex items-center space-x-2 space-y-0">
                                        <FormControl><RadioGroupItem value="GO" /></FormControl>
